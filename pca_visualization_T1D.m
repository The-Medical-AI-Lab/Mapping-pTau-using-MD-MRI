...
% Load the binary classification dataset of T1D
...
sgtitle(sprintf('PCA Component Loading Maps (T1D)', K));

% 6) Save figure (pixels = resolution × inches)
saveas(fig, 'image/pca/pca_loading_maps_T1D.png');
...
title('All Samples – Variance Explained (T1D)');
...
% Save figure
saveas(figVarAll, fullfile('image/pca/', 'PCA_variance_all_T1D_classification.png'));
...
%   nComp       : number of components explaining ≥95 % variance
    ...
    xlabel('Principal component index');
    ylabel('Individual variance (%)');
    title([tag ' – Variance Explained (T1D)']);
    ...
    % Save figure
    saveas(figVar, fullfile('image/pca/', sprintf('PCA_variance_%s_T1D_classification.png', lower(tag))));
    ...
    sgtitle([tag ' – Loading Maps (T1D)']);

    % Save figure
    saveas(figLoad, fullfile('image/pca/', sprintf('PCA_loadings_%s_T1D_classification.png', lower(tag))));
