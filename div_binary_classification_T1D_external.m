...
dataFile = "external dataset/classification_image_data_T1D_binary_external.mat";
S = load(dataFile);
...
for c = 1:C
    ...
    sgtitle('Average T_1-MD Joint Distributions and Difference (External)', 'FontName', 'Arial');
    
    set(gcf, 'PaperPositionMode', 'auto');
    outFile = fullfile(saveDir, 'PsiT1D_Average_binary_external.png');
...
% Save
targetFile = "external dataset/classification_image_data_T1D_binary_external.mat";
save(targetFile, 'KLD', 'JSD', '-append');
fprintf('Variables KLD and JSD have been appended to:\n%s\n', targetFile);
info = whos('-file', targetFile);
disp({info.name}');

targetFile = "external dataset/classification_data_T1D_binary_external.mat";
save(targetFile, 'KLD', 'JSD', '-append');
fprintf('Variables KLD and JSD have been appended to:\n%s\n', targetFile);
info = whos('-file', targetFile);
disp({info.name}');
...
yline(0,'k--');
title("T1D Jensen–Shannon Divergence Margin (pTau=1 − pTau=0)");
xlabel("Class (pTau=0, pTau=1)");
ylabel("JSD Margin");
set(fig1,'PaperPositionMode','auto');
print(fig1, fullfile(outDir,'Boxplot_JSD_Margin_T1D_external.png'), '-dpng','-r600');

% KLD margin
fig2 = figure('Units','inches','Position',[1 1 6 5],'Color','w');
boxplot(KLD_margin, y);
yline(0,'k--');
title("T1D Kullback–Leibler Divergence Margin (pTau=1 − pTau=0)");
xlabel("Class (pTau=0, pTau=1)");
ylabel("KLD Margin");
set(fig2,'PaperPositionMode','auto');
print(fig2, fullfile(outDir,'Boxplot_KLD_Margin_T1D_external.png'), '-dpng','-r600');
